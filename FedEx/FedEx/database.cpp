...
void commandForTables(int rc, sqlite3 *db, const char *sql, char *zErrMsg) {
...
int createDataBase()
{
	...
	/* Execute SQL statement */
	commandForTables(rc, db, sql, zErrMsg);
...
int insertData(int ID, std::string TableChoose, std::vector<int> results) {
	...
	if (TableChoose[0] == 'B') {
		...
		command = "INSERT INTO " + TableChoose + " (ID,NUMBER_ONE,NUMBER_TWO,NUMBER_THREE,NUMBER_FOUR, NUMBER_FIVE) "  \
		...
		command = "INSERT INTO " + TableChoose + " (ID,SHOOL,PARENTS,FAMILY,FRIENDS, UNICEF, INTERNET, TV, OTHER, NOWHERE) "  \
			"VALUES (" + std::to_string(ID) + ", " + std::to_string(results[0]) + ", " + std::to_string(results[1]) + "," \
			+ std::to_string(results[2]) + " , " + std::to_string(results[3]) + ", " + std::to_string(results[4]) +", "
			 + std::to_string(results[5]) + ", " + std::to_string(results[6]) + ", " \
			+ std::to_string(results[7]) + ", " + std::to_string(results[8]) + ");";
	} else if (TableChoose[0] == 'A') {
		command = "INSERT INTO " + TableChoose + " (ID,CR,POOR_CHILDREN,WORK_OF_UNICEF,INTERNET_BULLYING, HELPING, GAME, MOVIE, EVERYTHING, NONE_OF_THEM) "  \
			"VALUES (" + std::to_string(ID) + ", " + std::to_string(results[0]) + ", " + std::to_string(results[1]) + "," \
			+ std::to_string(results[2]) + " , " + std::to_string(results[3]) + ", " + std::to_string(results[4]) + ", "
			+ std::to_string(results[5]) + ", " + std::to_string(results[6]) + ", " \
			+ std::to_string(results[7]) + ", " + std::to_string(results[8])+");";
	} else if (TableChoose[0] == 'Q') {
		command = "INSERT INTO " + TableChoose + " (ID,NUMBER_ONE,NUMBER_TWO,NUMBER_THREE) "  \
			"VALUES (" + std::to_string(ID) + ", " + std::to_string(results[0]) + ", " + std::to_string(results[1]) + "," \
...
int eventID() 
{
	...
	SYSTEMTIME a;
	GetLocalTime(&a);
	//std::cout << "Date: " << a.wDay << "-" << a.wMonth << "-" << a.wYear << std::endl;
	std::string date_time = std::to_string(a.wYear)+"-" + std::to_string(a.wMonth)+"-" + std::to_string(a.wDay);
	int ID = (a.wYear % 100) * 100000000 + a.wMonth * 1000000 + a.wDay *10000 + a.wHour * 100 + a.wMinute;

	// Create SQL statement //
	std::cout << "DATE:" << date_time << std::endl;
	std::string command = "INSERT INTO WAKE_UP_HOUR (ID,DATE) " \
		"VALUES (" + std::to_string(ID) + ", '" + date_time + "');";
